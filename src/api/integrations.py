...
def receive_external_order(
    ...
    # 3. Pass to the FEFO Engine WITH BACKORDERS ALLOWED!
    try:
        db_order = create_order_with_fefo_reservation(
            db, 
            internal_order_schema, 
            allow_backorder=True # <--- THIS IS THE CRITICAL FIX
        )
        ...
        return {
            ...
            "status": db_order.status.name # Will correctly show BACKORDERED now
        }
